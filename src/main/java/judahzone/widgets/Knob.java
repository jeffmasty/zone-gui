...
//	}
	public void addListener(KnobListener listener) {
		...
		// Ensure knob can receive wheel events when inside wrapper panels
		setFocusable(true);
		...
		addMouseWheelListener(e -> {
			int notches = e.getWheelRotation();
			int delta = notches < 0 ? 5 : -2;
			int target = getValue() + delta;

			// clamp
			if (target > getMaxVal()) target = getMaxVal();
			if (target < getMinVal()) target = getMinVal();

			// Update the knob state first so getValue() reflects the change,
			// then notify the external listener (which may update model).
			setValue(target);
			listener.knobChanged(target);
		});
	}
	...
	public void paint(Graphics g) {
		...
		for(int i = 0; i < handles.size(); i ++){
			...
			if (knobColor == null) {
				ImageIcon thisIcon = thisHandle.icon;
				...
				g.fillOval(xc - 7, yc - 7, 14, 14);
				g.setColor(Color.BLACK);
				g.drawOval(xc - 7, yc - 7, 14, 14);
			}
		}
	 }
